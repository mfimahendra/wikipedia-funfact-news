...
fetch("http://localhost:9090/https://cinnabar.icaksh.my.id/api/public/daily/wiki")
...
searchWiki.on("keyup", function () {
    ...
    let search = $(this).val();
    if (search.length > 0) {
        fetch(`http://localhost:9090/https://${wikiLang}.wikipedia.org/w/api.php?action=opensearch&format=json&search=${search}`)
            .then(response => response.json())
            .then(data => {                
                $("#data-wiki").html("");                                
                data[1].forEach(ele => {
